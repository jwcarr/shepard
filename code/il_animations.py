...
def make_image(partition, cell_width=16, grid_width=1):
	width = partition.shape[0] * cell_width - grid_width
	height = partition.shape[0] * cell_width - grid_width
	image = np.full((width,height,3), 255, dtype=np.uint8)
	for (x, y), category in np.ndenumerate(partition):
		image[x*cell_width:((x+1)*cell_width)-grid_width,
			  y*cell_width:((y+1)*cell_width)-grid_width] = colors[category]
	return image

def save_image(partition, output_file, cell_width=16, grid_width=1):
	image = make_image(partition, cell_width, grid_width)
